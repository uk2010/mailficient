...
Use the **+** control beside **Favorites**, **New Folder…** in the main menu, or **Ctrl+Shift+N** to create a server folder. The Location picker includes every account root and existing custom folder, so the same dialog creates top-level folders and nested subfolders. Right-click anywhere in the sidebar for native **New Folder…**, **New Rule…**, and **Manage Rules…** actions. An account or folder click keeps the action scoped to that account; a custom folder also offers a direct subfolder action plus Favorites, rename, export, and confirmed deletion controls. Custom folder creation, renaming, and deletion are synchronized with the provider rather than stored as local-only decoration. Renaming a folder repairs rule and Quick Step references; deleting one warns when automation still points to it.
...
Press **Enter** in Search to request older matches from the selected **This Mailbox**, **This Account**, or **All Mail** server scope. Remote search is explicit, cancellable, and capped at 200 downloaded messages across the request; matching candidates join the private local cache and are then evaluated by the complete query. Ordinary typing never starts a network search.

Open **Rules…** from the menu at the right of the main window, or right-click the sidebar and choose **Manage Rules…**, to use the dedicated Rules window. **New Rule…** in the same sidebar menu opens the editor directly. Rules support multiple AND/OR conditions, exceptions, account scope, ordered actions, and **Stop processing**. Conditions cover sender, To/Cc/Bcc and Reply-To, subject/body, a mailbox picker, attachments and size, read/flag state, received date, labels, security status, mailing-list metadata, and raw headers. Actions can mark read/unread, flag/unflag or set a flag color, mark junk/not junk, archive, trash, apply/remove labels, move, or copy. Enable, disable, duplicate, edit, drag to reorder, or delete rules from one place. **Run Now** explicitly runs the selected rule—even while it is disabled for incoming mail—against a bounded pass of at most 10,000 cached messages. Select messages and choose **More → Apply Rules** to run every enabled rule in order, including stop-processing behavior, against that selection.

**Quick Steps** are named, reusable action sequences created on the Automation preferences page and run against the current message selection from **More → Quick Steps**. They use the same durable local operations as rules, including multi-message read, flag, label, move, and copy workflows. Labels can also be created and applied directly to one or many selected messages; search with `label:Name`.
