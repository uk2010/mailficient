...
The shield beside each message opens **Message Security** with the retained raw headers, bounded to 64 KiB, and any identity signals Mailficient can explain. A receiving-server SPF, DKIM, or DMARC failure, a different Reply-To or display-name domain, punycode, and a visible link label that opens another domain are surfaced conservatively. Passing authentication is context, not proof that a request, link, or attachment is safe. **Safe Senders** under **Preferences → Safety** load remote images automatically and hide the automatic sender-warning card. The complete assessment remains available from the shield, while link confirmation, attachment checks, and HTML restrictions stay active.
