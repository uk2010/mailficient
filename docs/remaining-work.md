...
13. Calendar-event regressions cover Today and Events service routing, input validation, timed and all-day VEVENT serialization, recurrence, and the EDS-backed creation boundary. Empty, light, dark, and keyboard states were reviewed.
...
5. Qualify Today and Events against an authorized CalDAV or GNOME Online Account on two devices. Create, edit, and delete timed, all-day, and recurring events in each client; verify that Mailficient and GNOME Calendar converge because Evolution Data Server remains the only event store.
