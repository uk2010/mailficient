...
docker run --detach --rm --name "$CONTAINER_NAME" \
  ...
  --env GREENMAIL_OPTS='-Dgreenmail.setup.test.all -Dgreenmail.hostname=0.0.0.0 -Dgreenmail.users=qa:mailficient-e2e@example.com -Dgreenmail.users.login=email' \
  "$IMAGE" >/dev/null
STARTED=1
...
printf '%s\n' 'GreenMail IMAPS/SMTPS/IDLE/draft/send/folder/server-search/reconnect qualification passed.'
