...
case "$STATE" in
  ...
  selection-refresh-stress) QA_MESSAGE=3; QA_SELECTION_REFRESH_STRESS=1 ;;
  error) QA_ERROR=1 ;;
...
DISPLAY="$CLIENT_DISPLAY" setsid dbus-run-session -- env GDK_BACKEND=x11 G_DEBUG=fatal-criticals NO_AT_BRIDGE=1 GTK_A11Y=none MAILFICIENT_QA=1 XDG_DATA_HOME="${MAIL_QA_DATA_DIR:-$QA_DATA_DIR}" \
  ...
  MAILFICIENT_QA_SELECTION_REFRESH_STRESS="${MAIL_QA_SELECTION_REFRESH_STRESS:-$QA_SELECTION_REFRESH_STRESS}" \
  MAILFICIENT_QA_SELECTION_REFRESH_STRESS_STEPS="${MAIL_QA_SELECTION_REFRESH_STRESS_STEPS:-}" \
  MAILFICIENT_QA_SELECTION_REFRESH_STRESS_INTERVAL_MS="${MAIL_QA_SELECTION_REFRESH_STRESS_INTERVAL_MS:-}" \
