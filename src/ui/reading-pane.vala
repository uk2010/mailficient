...
public class ReadingPane : Gtk.Box {
    ...
    public void show_message (Message message, Gee.List<Message>? conversation = null,
        ...
        vip.toggled.connect (() => {
            vip.icon_name = vip.active ? "starred-symbolic" : "non-starred-symbolic";
            vip.tooltip_text = vip.active ? "Remove sender from VIPs" : "Add sender to VIPs";
            Accessibility.label (vip, vip.tooltip_text); vip_toggled (message, vip.active);
        });
    ...
    private void populate_message_content (Message message, Gtk.Box message_content) {
        ...
        if (message.body_html != "") {
            ...
            html_view.layout_changed.connect (() => {
    ...
    private void clear () {
        ...
        foreach (var view in html_views) view.shutdown ();
        html_views.clear ();
        ...
        if (message.attachments.size == 0) {
            ...
            foreach (var attachment in message.attachments) {
                ...
                Accessibility.label (save, "%s %s, %s".printf (
                    ...
                    attachment.name, attachment.formatted_size ()));
                save.clicked.connect (() => choose_attachment_destination.begin (message, attachment, save));
                row.append (save);
                section.append (row);
            }
        }
